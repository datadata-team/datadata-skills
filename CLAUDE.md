...
## The five skills and how they divide up
...
- **`datadata-python`** — reference for writing **Python query scripts** (real Python in a RustPython/WASM sandbox, Polars-style DataFrame/Series/Expr, `query()`/`fetch()`). A third query-script type alongside SQL and DQL; this skill is the language/API reference. Its `references/builtins.pyi` is copied verbatim from the backend `lib/python-executor/builtins.pyi` (source of truth).
...
- **DQL source of truth**: `skills/datadata-dql/references/builtins.pyi` defines all built-in signatures. Every DQL built-in is a global — no `import`. When editing DQL docs, keep the `.pyi` and the `.md` reference files in sync.
- **Python source of truth**: `skills/datadata-python/references/builtins.pyi` is copied verbatim from the backend `lib/python-executor/builtins.pyi`. Injected globals (`query`/`fetch`/`args`/`print`/`pl`/`DataFrame`/`Series`) need no `import`; the API is Polars-style. Re-copy the `.pyi` when the backend changes, and keep the `.md` docs and `examples.md` in sync. `examples.md` scripts are verified by running them against the real executor (`internal/infra/python_executor` Go tests).
