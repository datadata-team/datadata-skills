...
本参考面向**生成独立 Python 脚本**的场景（爬虫、ETL、批处理），所有示例使用 `urllib.request`（仅标准库，零额外依赖）。
...
| 类型        | 方法 | 路径                                                | 权限                    |
| ----------- | ---- | --------------------------------------------------- | ----------------------- |
| Datasource  | GET  | `/api/v1/datasources/{id}/info`                     | `datasources:read`      |
| Datasource  | GET  | `/api/v1/datasources/{id}/list-tables`              | `datasources:read`      |
| Datasource  | GET  | `/api/v1/datasources/{id}/describe-table`           | `datasources:read`      |
| Datasource  | POST | `/api/v1/datasources/{id}/scan`                     | `datasource:scan`       |
| Datasource  | POST | `/api/v1/datasources/{id}/{schema}/{table}/comment` | `datasources:scan`      |
| Datasource  | GET  | `/api/v1/datasources`                               | `datasources:read`      |
...
payload = {
    ...
    "description": "Auto-generated API key for Datadata API",
}
