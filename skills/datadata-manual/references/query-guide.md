...
通过 MCP `execute-adhoc` 执行只读查询时，必须遵循以下 SQL 编写约定。
...
- 通过 `datasources` 参数绑定数据源，格式：`["DATASOURCE_ID:ATTACH_ALIAS"]`，可绑定多个
- SQL 中通过别名而非 datasource ID 引用表
- `dataspace` 类型数据源以只读方式挂载为普通 DuckDB 数据库，同样通过别名引用（惯例上用 datasource 的 `name` 作为别名，通过 `get-datasource-info` 获取）
...
别名惯例上复用 datasource 的 `name`（通过 `get-datasource-info` 获取），此时即 `"{datasourceName}".main."{tableName}"`。

> **Data Spaces 表管理**（建表、写入、删表）不走 `execute-adhoc`，而是用 `dataspace-execute-sql` 执行任意 DuckDB SQL。详见 [data-spaces.md](./data-spaces.md)。
...
- 写入 dataspace 用 `dataspace-execute-sql`（DDL/DML 均可），详见 [data-spaces.md](./data-spaces.md)
- 未明确要求时不要运行破坏性 SQL
- 不要默默改写业务逻辑 SQL

## 结果处理

MCP 的 `execute-adhoc` 返回执行 ID 和下载链接（NDJSON/CSV）：

- 不要将完整的大数据集发送到 model context — Agent 用 `curl` 下载到 `/tmp/`，用 `head`/`tail` 预览
- 报告文件路径、格式和 execution ID 以便重用
- 大数据集优先用 `grep`、`awk`、`jq` 等命令行工具本地处理
